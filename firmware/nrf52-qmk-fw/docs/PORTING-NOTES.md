...
- **키보드 로직 = QMK** (`quantum/` vendor-in, **우리는 고치지 않는다**) — action/layer/tapping/VIA/dynamic_keymap
  - [정확히는] 베이스는 **upstream QMK 가 아니라 baram-qmk-8k 의 스냅샷**이다. 그 포크에는 baram 의
    훅이 이미 들어 있다 — 예: `debounce/sym_defer_pk.c` 의 `#ifdef DEBOUNCE_RUNTIME` ->
    `debounce_time_get()`(upstream 엔 없다). upstream 과 diff 하면 `quantum.c`/`keyboard.c`/
    `action.c`/`via.c` 등 대부분이 다르다(버전 드리프트 + 포크 패치). "순정" 이 아니라
    **"vendor-in 트리는 우리가 건드리지 않는다"** 가 우리 원칙이다.
- **하드웨어·저전력 = Zephyr 네이티브** (kbd-matrix, USB, 향후 BLE/PM)
- **접착 계층 = `port/`** (우리가 작성하는 어댑터)

```
QMK quantum/ (baram-qmk 스냅샷, GPLv2, 우리는 무수정)
...
### 2.11 런타임 노브 (VIA) — 디바운스 시간 / HOLD_ON_OTHER_KEY_PRESS

`port/via/debounce_cfg.c`, `port/via/hold_okp.c`. VIA 메뉴는 **FEATURE > QMK**
(채널 17 = 디바운스, 18 = hold-okp). EEPROM 오프셋은 `port/port.h` 의 +8 / +12.

디바운스 UI 는 **드롭다운**이다(baram-qmk VENOM 과 동일한 5ms 단위: 5/10/.../40). 값은 인덱스가
아니라 실제 ms 다. **기본값은 반드시 옵션 목록 안에 있어야 한다** — 아니면 공장초기화 직후 VIA
드롭다운이 "선택 없음"으로 뜬다(10 을 쓰는 이유 중 하나).

**디바운스는 시간만 런타임이다**(사용자 결정). 알고리즘 타입은 `config.cmake` 의 `DEBOUNCE_TYPE`
로 빌드타임 고정(`sym_defer_pk`). baram-qmk(VENOM)는 타입도 바꾸지만, 그러려면 두 알고리즘을
다 링크하고 순정 `debounce()` 를 우회하는 층(`debounce_runtime.c`)이 필요하다 — 타입이 고정이면
낼 이유가 없는 비용이다.

**함정 1 — 컴파일타임 `DEBOUNCE` 는 여전히 필요하다.** `sym_defer_pk.c` 는 `#if DEBOUNCE > 0`
으로 구현 전체를 가둔다. 0 이면 파일이 통째로 패스스루가 되어 **런타임 값이 무의미**해진다.
config.h 의 `DEBOUNCE` 는 런타임 값과 무관하게 0 보다 커야 한다(지금 10).

**함정 2 — 런타임 값 0 은 디바운스를 무력화한다.** `eeconfig_init_user_datablock()` 이 사용자
영역을 0 으로 밀므로(§port.h 규칙) EEPROM 이 비면 0 이 읽힌다. 그대로 쓰면 카운터가
`DEBOUNCE_ELAPSED` 로 로드돼 **채터링이 그대로 올라온다**. init 과 VIA set 양쪽에서 클램프한다
(5~40ms). hold-okp 는 0(=OFF)이 유효값이라 magic 으로 "저장된 적 있음"을 판별한다.

**함정 3 — `HOLD_ON_OTHER_KEY_PRESS_PER_KEY` 가 없으면 VIA 토글이 조용히 무효다.**
`action_tapping.c` 는 그 매크로가 있을 때만 `get_hold_on_other_key_press()` 를 부른다. 없으면
`TAP_GET_HOLD_ON_OTHER_KEY_PRESS` 가 상수 false 로 굳는다 — 컴파일도 되고 토글도 저장되는데
동작만 안 한다. 키보드 config.h 에 반드시 둘 것.

**함정 4 — 원본이 `debounce_time_get()` 을 선언 없이 부른다.** `quantum/debounce.h` 엔 선언이
없어 그냥 컴파일하면 **암묵적 선언**(반환 타입 int 추정)이 된다. 선언을 vendor-in 트리에 넣는
대신 `port/debounce/debounce_wrapper.c` 가 선언을 먼저 넣고 원본을 `#include` 한다(baram 의
`port/debounce_defer_pk.c` 와 같은 방식). CMake 는 그때 **원본을 소스 목록에서 뺀다** — 둘 다
컴파일하면 `debounce()` 가 중복 정의된다. 래퍼를 `port/debounce/` 하위에 둔 이유는 glob 이
`port/*.c`(비재귀)라 같은 파일이 양쪽에 잡히는 것을 피하기 위해서다.
...
**`col-drive-inactive` 가 필수다 (실기기에서 겪음)**. 없으면 부팅 즉시 죽는다:

```
<err> input_gpio_kbd_matrix: Pin 0 configuration failed: -134   (-ENOTSUP)
```

Zephyr `gpio-kbd-matrix` 의 **기본 동작은 선택 안 된 컬럼을 `GPIO_INPUT`(하이임피던스)로 두는 것**이다.
595 는 출력 전용이라(읽기 회로가 없다) 그 요청을 거부한다 — 우리 드라이버의 `-ENOTSUP` 이 그대로
올라온 것이다. 이 속성을 주면 컬럼을 항상 출력으로만 다룬다(`init`: `GPIO_OUTPUT_INACTIVE`,
`구동`: `gpio_pin_set_dt()`). 다이오드가 있으므로 비활성 컬럼을 LOW 로 구동해도 전기적으로 안전하다.

**덤: `direct_write`** — 이 속성 + 컬럼이 coherent(같은 포트·같은 dt_flags·연속 핀)면 드라이버가
`gpio_port_set_masked()` **한 번**으로 컬럼 전체를 쓴다. `<&shift_reg 0..15>` 가 그 조건을 만족한다.
스캔 스텝당 SPI 1회다 — 핀별 경로였다면 2회(켤 핀 + 끌 핀). 595 의 "부분 갱신 불가"(항상 체인
전체를 쏜다)와도 궁합이 맞는다.
...
| 6 | VIA 커스텀 메뉴 — 타임아웃 2개 | ✅ 실기기 |
| 7 | 다중 보드 구조화 — 보드 독립성 리팩터 | ✅ 실기기(wish60 회귀 확인) |
| 7 | **wish65 추가 (nRF52833 + 595 16컬럼) — 전 기능 동작** | ✅ 실기기(아래 표) |
| 8 | ws2812 + QMK RGB_MATRIX + VIA RGB + ext-power | ✅ 실기기(RGB 65.08mA, idle 소등) |
| — | 런타임 디바운스 시간 + HOLD_ON_OTHER_KEY_PRESS (VIA) | ⬜ 빌드만 |
| — | BLE 이름에 주소 유래 16비트 해시 | ⬜ 빌드만 |

**Phase 7 검증 결과 — 두 트리 구조가 성립한다.** 두 번째 보드가 **다른 SoC + 시프트레지스터
GPIO 확장 + 다른 LED 개수 + 다른 매트릭스 방향**으로 들어왔는데, 공통 코드는 손대지 않았다
(`col-drive-inactive` 한 줄과 `led.h` no-op 경로가 전부다).

| wish65 검증 항목 | 결과 |
|---|---|
| nRF52833 부팅 + 파티션(512KB 재배분) | ✅ |
| **595 16컬럼 스캔** (`col-drive-inactive` 필수 — §4.2) | ✅ |
| 키맵 방향 (`MATRIX_DRIVE_IS_QMK_COL`, col2row) | ✅ 전치 없음 |
| BLE 연결 + 무선 타이핑 | ✅ (LFXO 가정 확정) |
| Caps LED (P1.09) | ✅ |
| RGB 18개 (VIA 로 on/off) | ✅ |
| VIA 로딩 (`WISH65-VIA.JSON`, 65% 67키) | ✅ |
| idle 전력 | **38.09µA** (wish60 보다 낮다 — §6.11) |
| 키 눌림 전력 | 3.18mA (§6.11) |
...
- **전력**: wish60 스트립은 16개. WS2812 는 검은색을 표시해도 컨트롤러가 개당 ~0.7mA →
  **약 11mA, idle 80.9µA 의 140배**. ext-power 게이팅 없이는 성립 자체가 안 된다.
  (ZMK 도 wish60 에 `RGB_UNDERGLOW_ON_START=n` 을 건다. **`AUTO_OFF_IDLE`/`AUTO_OFF_USB` 는 안 건다** —
  아래 §6.12 참고. 예전에 여기 "ZMK 도 건다" 고 적어뒀는데 사실이 아니었다)
...
### 6.11 wish65(595 16컬럼) 스캔 전력 — 분해 실측

키를 누른 채, BLE 연결, USB 미연결. wish60 과 달리 컬럼 구동이 **SPI 트랜잭션**이고 컬럼이
5개가 아니라 **16개**다. 스캔 비용은 매트릭스 크기가 아니라 **구동 컬럼 수**에 비례한다.

| 구성 | 전류 | Δ |
|---|---|---|
| settle 50µs(기본), QMK 2ms | 3.83mA | — |
| settle **20µs**, QMK 2ms | **3.18mA** | -0.65 (settle) |
| settle 20µs, QMK 4ms | 2.81mA | -0.37 (QMK 루프) |
| **채택: settle 20µs, QMK 2ms** | **3.18mA** | 디바운스 분해능을 위해 0.37 을 낸다 |

**idle 은 오히려 wish60 보다 낮다 — 38.09µA** (BLE 연결, 키 없음, 10초 평균).

| | idle | 이유 |
|---|---|---|
| wish60 | ~60µA | 기판에 **MAX17048 이 실장돼 있다** |
| **wish65** | **38.09µA** | MAX17048 없음(VDDH 직접 측정 + TP4054 충전기뿐) |

**우리 펌웨어는 두 보드 다 `maxim,max17048` 노드를 켜지 않는다**(생성 DTS 에 0건). 그런데도
차이가 나는 건 **칩이 물리적으로 붙어 있으면 대화를 안 해도 대기 전류를 먹기 때문**이다 —
§6.5 의 "deep sleep 27.7µA 중 ~25µA 는 MCU 가 아니라 MAX17048" 과 같은 이야기다.
`60 - 25 = 35µA` vs 실측 38µA 로 거의 맞는다.

**595 는 idle 에 기여하지 않는다.** CMOS 래치라 상태가 고정되면 정적 전류가 nA 수준이다.
"595 가 상시 전원이니 누설이 있을 것"은 기우였다(§4.2 의 "595 전원 유지" 걱정과 같은 결말).

파형도 아는 얼굴이다 — 촘촘한 12.7mA 는 **345ms** 간격(BLE 연결 이벤트, `11.25ms × 31 = 348ms`),
큰 17.7mA 3발은 **3초** 간격(§6.8 의 호스트 LED 폴링). 38.09µA 중 **~11µA 가 호스트 몫**이고
그게 없으면 **~27µA** 다.

**따라서 wish65 의 deep sleep 은 wish60 의 27.7µA 보다 훨씬 낮을 것이다**(그 27.7 의 대부분이
MAX17048 이었으므로). 미측정.

**`settle-time-us` 가 컸다.** 드라이버는 컬럼 구동 후 `k_busy_wait(settle_time_us)` 로 **CPU 를 풀
전류로 돌리며** 기다린다(`input_kbd_matrix.c`). 기본값 50µs × 16컬럼 = 스캔당 800µs — poll-period
4ms 의 20% 다. 20µs 로 낮춰 0.65mA 를 얻었다. 근거: row 는 내장 pull-down(~13kΩ) + 트레이스/다이오드
용량(~100pF) 이라 τ≈1.3µs, 99% 정착에 ~6.5µs. 20µs 는 3배 마진. **더 줄이면 정착 전에 읽어 키가
씹히거나 유령키가 뜬다** — 줄일 땐 반드시 실기기 타이핑 확인.

**남은 비용은 SPI 오버헤드지 전송이 아니다.** 3.18mA 기준 듀티 ~47%(4ms 중 1.87ms 활성), 그중
settle 320µs 를 빼면 SPI 16회에 ~1.55ms = **회당 ~97µs**. 2MHz 에서 2바이트 전송은 **8µs** 뿐이다.
나머지 ~89µs 는 Zephyr SPI 스택(블로킹 `spi_write_dt` 의 세마포어 + 컨텍스트 스위치, `spi_context_lock`
뮤텍스, CS GPIO). **SPI 클럭을 올려도 8µs -> 2µs 라 ~0.16mA 뿐이다** — 안 했다.

**settle 을 재우는 것은 불가능하다.** 커널 틱이 32768Hz = **30.5µs** 라 20µs 를 `k_sleep` 하면
오히려 30.5µs 를 자고 슬립/웨이크 오버헤드까지 붙는다. busy-wait 이 더 싸다. (스캔 **사이**엔 이미
잔다 — 파형의 0 근처 53% 가 그것)

**§6.4 표를 오독하지 말 것.** wish60 의 "주기 2배마다 ~1mA" 는 **매트릭스 주기와 QMK 주기를 함께**
바꾼 값이다. QMK 루프 단독은 여기 실측대로 0.37mA 다 — 대부분이 매트릭스 스캔 몫이었다.

**이걸 더 팔 가치가 있나.** 3.18mA 는 **키를 계속 누른 최악치**다. 타이핑 8h + 방치 16h 면 평균
~1.1mA -> 1000mAh 로 **약 38일**이고, 실제 타이핑은 키 사이에 인터럽트 모드로 빠져 더 좋다.
**RGB 를 켜면 65mA / 2일**(§6.10)이다 — 스캔 전력은 배터리 수명을 좌우하지 않는다.

### 6.12 idle 타임아웃 = 사실상 "RGB 타임아웃" — ZMK 를 따르지 않는다

**ZMK 는 이 보드들에서 RGB 를 idle 에 끄지 않는다.** 확인한 사실:
- `zmk-config` 의 wish60/wish65 defconfig 에 `AUTO_OFF_IDLE`/`AUTO_OFF_USB` 가 **없다**.
- ZMK Kconfig 에도 `default` 가 없다 -> 기본 **n**.
- 즉 ZMK 에서 RGB 는 deep sleep 전까지 켜져 있다.

(예전에 이 문서에 "ZMK 도 AUTO_OFF_IDLE 을 건다" 고 적고 Phase 8-C 의 근거로 삼았는데
**사실이 아니었다**. 지어낸 근거였다.)

**그래도 우리는 끈다.** ZMK 에도 그 옵션은 **존재한다** — 쓸지 말지의 문제지 없는 기능이 아니고,
숫자가 다르다: RGB 65mA vs idle 60µA(§6.10). 켠 채로 두면 1000mAh 가 15시간이다.

**다만 타임아웃은 ZMK 값을 쓰면 안 됐다.** 처음엔 `CONFIG_ZMK_IDLE_TIMEOUT=30초`를 그대로
가져왔는데 **용도가 다른 값**이다. ZMK Kconfig 가 직접 말해준다:
...
**"OLED shutoff, etc"** — 디스플레이용이다. 실제 IDLE 구독자:

| ZMK 구독자 | IDLE 에 하는 일 |
|---|---|
| `display/main.c` | `stop_display_updates()` — OLED 갱신 중단 |
| `battery.c` | `k_timer_stop(&battery_timer)` — 배터리 폴링 중단 |
| `rgb_underglow.c` / `backlight.c` | **`AUTO_OFF_IDLE` 을 켜야만** 반응 (이 보드들은 안 켠다) |

**wish60/wish65 엔 OLED 가 없다.** 그래서 ZMK 에서 그 30초가 하는 일은 배터리 폴링 중단이
전부다 — 무해한 값이라 보드 defconfig 도 Kconfig 기본값(30000)을 그대로 적어뒀을 뿐이다.
(반면 sleep 은 ZMK 기본 15분에서 **1시간으로 바꿨다** — 그건 의도적 선택이다)

우리는 그 30초를 가져다 **"RGB 소등"이라는 전혀 다른 일**을 시켰다. OLED 를 끄기엔 30초가
적당하지만(다시 켜면 그만) RGB 를 끄기엔 짧다. **값을 빌려올 때 그 값이 무엇을 위한 것인지
확인하지 않은 것이 실수다.** 우리 IDLE 은 `qmkSuspendUpdate()` 가 유일한 소비자라 사실상
RGB 타임아웃이므로, 그에 맞게 정해야 한다.

**기본값 30초(사용자 결정).** 2분으로 올려봤다가 되돌렸다 — RGB 는 65mA 라 idle 60µA 의 1000배라
자리를 뜨면 빨리 끄는 편이 낫다는 판단이다. (참고로 길게 잡아도 비용은 작다: 65mA × 4분 =
4.3mAh = 1000mAh 의 0.4%. 진짜 절감은 그 뒤 **계속** 꺼져 있는 데서 나온다)

짧다고 느끼면 VIA(FEATURE > POWER > Idle Timeout)에서 Off~4분으로 올리면 된다.
**Off 를 고르면 ZMK 와 같은 동작**(deep sleep 전까지 RGB 유지)이 된다.

즉 값은 ZMK 와 우연히 같아졌지만 **이유가 다르다** — ZMK 의 30초는 OLED 용이고 RGB 와 무관하다.
우리 30초는 RGB 소등을 위해 고른 값이다.

**[주의] 기본값을 바꿔도 EEPROM 에 저장된 값이 있으면 그게 이긴다**(`power_cfg_init`). 이미 쓰던
보드는 VIA 에서 직접 바꿔야 한다.
