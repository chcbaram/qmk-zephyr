...
→ `CONFIG_PM_DEVICE=y` + `CONFIG_PM_DEVICE_RUNTIME=y` + DTS spi3 에 `zephyr,pm-device-runtime-auto`.
드라이버가 전송 전후로 `pm_device_runtime_get/put` 을 부르므로 **전송 사이에 알아서 suspend** 한다.
**spi3 에만** 걸린다(확인: runtime-auto 노드는 spi3 + SoC 기본값인 i2c0/i2c1/adc 뿐).
